		...
		if(detect == 1) 
		{
			...
			for (size_t i = 0; i < heart.size(); ++i)
			{
				if(heart[i] == 0)
				{
					...
					cout<<"Finished telling everyone that "<<dead_node<<" died"<<endl;
				}
...
void * chord::get_command(void * t)
{
	while(1)
	{
		...
		int idx_createnode = cmd.find("createnode");
...
void chord::createnode(int id)
{
	...
	request_updateFT_createnode(id);
	updateKeys(id);
...
void chord::request_updateFT_createnode(int id)
{
	...
	for(int i = 0; i<M; i++)
	{
		...
		int pred_id = nodes[findpredidx_createnode(num) ].id;
...
int chord::findpredidx_createnode(int id)
{
	int pred_idx = nodes.size() - 1;
	for(size_t i = 0; i<nodes.size(); i++)
	{
...
int chord::server_response(chord * C, int id, int socket)
{
	...
	int idx_createnode = response.find_first_of("j");
